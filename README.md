...
7. 측정 유형에서 **독립 질문** 또는 **연속 대화**를 선택한다.
8. **측정 시작**을 누른 다음 새 질문을 입력한다.
9. 다음 질문은 **새 채팅 → 새 대화로 분리 → 임시 채팅 전환 → 질문** 순서로 진행한다.
10. 답변이 끝나면 **측정 중지**, **JSON 내려받기**를 차례로 누른다.
...
여러 독립 질문을 같은 조건으로 반복 측정하려면 사이드패널의 **측정할 질문**에 질문을 한 줄에 하나씩 입력하고, 반복 횟수를 선택한 뒤 **질문 준비**를 누른다. 별도 시스템에서 만든 질문 세트는 **JSON으로 가져오기(고급)**에서 [예제 JSON](examples/query-set.example.json) 형식으로 불러올 수 있다.

1. 첫 질문에서 **측정 시작**을 누른다.
2. **질문 복사** 후 ChatGPT에 붙여 넣어 전송한다.
3. 질문이 수집되면 사이드패널에 다음 질문이 자동으로 표시되므로 미리 복사한다.
4. 답변이 끝나면 ChatGPT에서 **새 채팅**을 연다. 완료 여부는 사용자가 판단하며 Collector의 추정값이 버튼을 막지 않는다.
5. 사이드패널에서 **다음 질문 준비**를 누르고 ChatGPT의 **임시 채팅**을 켠 다음 복사한 질문을 전송한다.
...
- 사용자가 확정한 임시 채팅 경계를 `conversation_instance_id`로 기록
