...
- 계정 플랜과 모델 선택 방식이 측정 시작 전에 표시되고, 사이드패널을 다시 열어도 마지막 선택값이 유지된다.
...
- 기본 모델 측정은 `model_selection: default`, 사용자가 모델을 직접 고른 측정은 `model_selection: manually_selected`로 기록된다.
